...
export interface DiscordConfig {
	...
	timestamps: boolean;
	/** The Nocturne badge in the artwork's corner. Needs `cover`: on its own `small_image` is not a
	...
	button1: string;
	button2: string;
	/** Custom registered Discord Application ID (optional, defaults to Nocturne official). */
	app_id?: string;
	/** Custom button label for the link button. */
	custom_button_label?: string;
}

/** Must match `impl Default for RpcConfig` — the default card plus the provider badge. */
export const DISCORD_DEFAULTS: DiscordConfig = {
	...
	button2: 'app',
	app_id: '',
	custom_button_label: 'Listen on Nocturne'
};
...
export function cardText(slot: string, t: PreviewTrack): string | null {
	...
	switch (slot) {
		...
		default: {
			...
			const res = slot
				.replace(/\{title\}/gi, t.title || '')
				.replace(/\{artist\}/gi, t.artists || '')
