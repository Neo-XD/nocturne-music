 ...
 */
let scratchToHex: CanvasRenderingContext2D | null | undefined;
function toHex(color: string): string {
	...
	if (scratchToHex === undefined) {
		const c = document.createElement('canvas');
		c.width = c.height = 1;
		scratchToHex = c.getContext('2d', { willReadFrequently: true });
	}
	const ctx = scratchToHex;
