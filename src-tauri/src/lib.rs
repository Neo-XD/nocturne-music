    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
...

            {
                ...
                tauri::async_runtime::spawn(async move {
                    // Set before the enabled check so settings can show the PIN the server will accept while it is stopped.
                    rs_ctrl.set_pairing_pin(crate::remotesync::ensure_pairing_pin(&db_sync)).await;
                    if db_sync.get_setting("remote_sync_enabled").as_deref() != Some("false") {
                        let port = db_sync
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_remote_sync_paired_devices,
            commands::rename_remote_sync_device,
            commands::set_remote_sync_device_favorite,
            commands::remove_remote_sync_device,
            ...
            commands::get_downloaded_songs,
            commands::scan_downloaded_songs,
            ...
            match ev {
                PlayerEvent::Position(p) => {
                    if throttle.should_emit(p, std::time::Instant::now()) {
                        ...
                        if let Some(rs) = app
                            .try_state::<std::sync::Arc<crate::remotesync::RemoteSyncController>>()
                        {
                            if rs.is_running() {
                ...
                PlayerEvent::Duration(d) => {
                    ...
                    if let Some(rs) =
                        app.try_state::<std::sync::Arc<crate::remotesync::RemoteSyncController>>()
                    {
                        if rs.is_running() {
                            let snapshot = state.playback_snapshot().await;
                            let st = crate::remotesync::room_state_from_snapshot(&snapshot);
