...
- **Embedded Remote Playback Server (Mobile Control Hub)**: Added a built-in local WebSocket remote sync server with secure PIN authentication and pairing QR code in Settings, allowing you to control desktop playback from any mobile device or browser on your local network.
- **Discord Rich Presence Update**: Configured official Nocturne Discord application credentials so Discord activity displays "Listening to Nocturne" with high-resolution artwork and time tracking.
...
- **Playlist Banner Gradient Masking**: Replaced harsh banner borders with a smooth multi-stop cubic gradient alpha mask fading seamlessly into the page background.
...
async fn refresh_spotify_dev_token(state: &AppState) -> Result<String, String> {
    ...
    if refresh_token.is_empty() || client_id.is_empty() {
        return Err("Spotify Developer credentials missing or expired. Please re-link in profile."
            .to_string());
    }
    ...
    if let Some(items) = json["items"].as_array() {
        for item in items {
            ...
            let subtitle = match track_count {
                Some(count) if count > 0 => Some(format!(
                    "{count} {} • {owner}",
            ...
            if res.status().is_success() {
                if let Ok(pl_json) = res.json::<serde_json::Value>().await {
                    ...
                    let mut items_res = client
                        .get(format!(
                            "https://api.spotify.com/v1/playlists/{clean_id}/items?limit=100"
                        ))
                        .header("Authorization", format!("Bearer {token}"))
                        .header(
                            "User-Agent",
                            ...
                            if let Ok(items_json) = ir.json::<serde_json::Value>().await {
                                if let Some(items_arr) = items_json["items"].as_array() {
                                    for entry in items_arr {
                                        ...
                                        let track_id =
                                            track["id"].as_str().unwrap_or_default().to_string();
                                        let name =
                                            track["name"].as_str().unwrap_or_default().to_string();
                                        if name.is_empty() {
                                            continue;
                                        }
                                        ...
                                        let album =
                                            track["album"]["name"].as_str().map(|s| s.to_string());
                                        let duration_ms =
                                            track["duration_ms"].as_u64().unwrap_or(0);
    ...
    if let Ok(res) = embed_res {
        if res.status().is_success() {
            if let Ok(html) = res.text().await {
                if let Some(start) =
                    html.find("<script id=\"__NEXT_DATA__\" type=\"application/json\">")
                {
                    let json_start =
                        start + "<script id=\"__NEXT_DATA__\" type=\"application/json\">".len();
                        ...
                        if let Ok(json) = serde_json::from_str::<serde_json::Value>(json_str) {
                            ...
                            if let Some(track_list) = entity["trackList"].as_array() {
                                for t in track_list {
                                    ...
                                    let artists =
                                        t["subtitle"].as_str().unwrap_or_default().to_string();
                                    let duration_ms = t["duration"].as_u64().unwrap_or(0);
                                    let preview_url = t["audioPreview"]["url"]
                                        .as_str()
                                        .unwrap_or_default()
                                        .to_string();
...
pub async fn spotify_transfer_to_ytm(
    ...
    let desc = playlist_info
        .description
        .unwrap_or_else(|| "Transferred from Spotify to Nocturne / YouTube Music".to_string());
    ...
    for t in playlist_info.tracks {
        ...
        if let Ok(results) = state.it.search_songs(ytm_client, &search_query).await {
            if let Some(first) = results.items.first() {
                let _ =
                    state.it.playlist_add(ytm_client, &new_ytm_playlist_id, &first.video_id).await;
